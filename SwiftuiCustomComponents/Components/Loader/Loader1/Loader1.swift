...
struct Loader1: View {
    ...
    // MARK:- variables
    let circleTrackGradient = LinearGradient(gradient: .init(colors: [Color.blue.opacity(0.4), Color.blue.opacity(0.4)]), startPoint: .leading, endPoint: .bottomLeading)
    let circleRoundGradient = LinearGradient(gradient: .init(colors: [Color.blue, Color.blue]), startPoint: .topLeading, endPoint: .trailing)
    ...
    func animateLoader() {
        ...
        Timer.scheduledTimer(withTimeInterval: animationDuration, repeats: false) { _ in
            withAnimation(Animation.easeInOut(duration: self.trackerRotation * self.animationDuration)) {
                self.rotationDegree += self.getRotationAngle1()
            }
        }
...
struct Loader1_Previews: PreviewProvider {
